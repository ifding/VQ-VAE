...
def training_plot(scores, folder_name, file_name):
    ...
    plt.plot(range(len(scores["train"])), scores["val"], label=f'val {file_name}')
    plt.title(f'{file_name} plot'); plt.xlabel('Epoch'); plt.ylabel(f'{file_name}');
    plt.legend(); 
    #plt.show()
    plt.savefig("{}/{}.png".format(folder_name, file_name), dpi=300)
    

def dict_to_csv(d, folder_name, file_name):
    df = pd.DataFrame.from_dict(d, orient='index')
    df = df.transpose()
    df.to_csv("{}/{}.csv".format(folder_name, file_name), index=False)
